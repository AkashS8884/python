...
multi_class='auto')
 ...
 sobelxy = cv2.Sobel(src=img_blur, ddepth=cv2.CV_64F, dx=1, 
dy=0, ksize=5)
 return np.array(sobelxy)
 ######
 opencvImage = 
cv2.cvtColor(np.array(stringToImage(my_uploaded_file_base64)), 
cv2.COLOR_RGB2BGR)
 norm_image = cv2.normalize(opencvImage, None, alpha=0, beta=1, 
...
ksize=5)
 ...
 result = clf.predict(np.array([[-0.91318,-2.0113,-
0.19565,0.066365]]))
...
{'original_image':imagedata.getvalue(),
